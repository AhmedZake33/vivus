<template>
  <div v-if="$route.params.system > 0 && $route.params.system < 4">
    <DataTable
      :items="$store.getters['finance/submissions']"
      @Refresh="Refresh"
      :loading="$store.getters['finance/submissionsLoad']"
      :name="$route.params.system == 1 ? $t('Global.finance_submissions') : $route.params.system == 2 ? $t('Global.pre_finance_submissions') : $t('Global.TR_finance_submissions')"
      ...
      :period="period"
      :is_credit="is_credit"
      :system="Number($route.params.system)"
    ...
    >
      ...
      <template v-if="$route.params.system != 3" #filter_table>
        <div v-if="$route.params.system == 1" class="ml-2 mr-2 mb-2 fullWidth">
        ...
        <div class="ml-2 mr-2 mb-2 fullWidth">
          <el-select clearable :no-data-text="$t('Global.nodata')" v-model="is_credit"   :placeholder="$t('Global.pay_type')" style="width: 100%">
            <el-option  :label="$t('Global.baseType')" :value="1">
              <span style="float: right; color: #8492a6; font-size: 13px">
               {{ $t('Global.baseType')}}
              </span>
            </el-option>
            <el-option  :label="$t('Global.combinedPayment')" :value="2">
              <span style="float: right; color: #8492a6; font-size: 13px">
      ...
      <template v-if="$route.params.system != 3" #cols>
        <el-table-column prop="id" sortable width="54" label=""></el-table-column>
        ...
        <el-table-column v-if="$route.params.system == 1" prop="type" :label="$t('Global.type')"></el-table-column>
        ...
        <el-table-column prop="credit" :label="$t('Global.pay_type')">
          <template slot-scope="scope">
              <div style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap; width: 100%">
                <CBadge color="info" class="ml-1">
                  {{ scope.row.credit == 1? $t('Global.baseType') :  $t('Global.combinedPayment') }}
                </CBadge>
              </div>
          </template>
        </el-table-column>
        <el-table-column v-if="$route.params.system == 1" prop="status" width="100" :label="$t('Global.status')">
          <template slot-scope="scope">
            <CBadge color="info" class="ml-1">{{ scope.row.status ? $t(`Global.state_${scope.row.status}`) : $t(`Global.state_1`) }} </CBadge>
          </template>
        </el-table-column>
        <el-table-column prop="starts_at" :label="$t('Global.visit')">
          <template slot-scope="scope">
            <div>
              <CBadge color="success" class="ml-1">
                {{ scope.row.starts_at ? DateFormat(scope.row.starts_at) : `${$t('Global.noAction')}` }}
              </CBadge>
              <CBadge color="warning" class="ml-1">
                {{ scope.row.ends_at ? DateFormat(scope.row.ends_at) : `${$t('Global.noAction')}` }}
              </CBadge>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="type" width="300" :label="$t('Global.details')">
          <template slot-scope="scope">
            <span>{{ $t('Global.pay_number') + ': ' }} </span>
            <span style="font-weight: bolder" class="ml-3"> ({{ scope.row.pay_number ? scope.row.pay_number : '-' }}),</span>
            <span>{{ $t('Global.payment_date') + ': ' }} </span>
            <span style="font-weight: bolder" class="ml-3"> ({{ scope.row.payment_date ? toLocalDatetime(scope.row.payment_date, `${$i18n.locale == 'ar' ? 'ar' : 'en'}`) : '-' }})</span>
          </template>
        </el-table-column>
        <el-table-column prop="id" width="80" label=" ">
          <template slot-scope="scope">
            <div :style="$i18n.locale === 'ar' ? 'text-align:left' : 'float: right;'">
              <router-link :to="{ path: `/finance/show/${$route.params.system}/${scope.row.id}` }">
      ...
      <template v-else #cols>
        <el-table-column prop="id" sortable width="65" label="#"></el-table-column>
        <el-table-column prop="course" :label="$t('Global.name')" sortable width="200">
          <template slot-scope="scope">
        ...
        <el-table-column prop="trainee" :label="$t('Global.trainee')" sortable>
          <template slot-scope="scope">
            <el-tooltip effect="dark" placement="top">
              <div slot="content">
        ...
        <el-table-column prop="price" :label="$t('Global.coursePrice')" sortable>
          <template slot-scope="scope">
            <el-tooltip effect="dark" placement="top">
              <div slot="content">
        ...
        <el-table-column prop="start" :label="$t('Global.Date')" sortable>
          <template slot-scope="scope">
            <div>
              <CBadge color="success" class="ml-1">
              ...
              <CBadge color="warning" class="ml-1">
                {{ scope.row.end ? DateFormat(scope.row.end) : `${$t('Global.noAction')}` }}
              </CBadge>
            </div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    Refresh(query) {
      this.$store.dispatch('finance/submissions', { query: query }).then(_ => {
        this.isLoad = false;
      });
    },
  },
