  ...
  <CRow v-if="hasPermission('edit_organizations')" class="edit_organization">
        ...
        <CCardBody v-loading="$store.getters['organization/organizationLoad']" dir="rtl" style="text-align: initial">
          <el-form label-position="top" :rules="rules" :model="ruleForm" status-icon size="large" ref="ruleForm" label-width="120px" class="demo-ruleForm">
            <CRow>
              ...
              <CCol sm="4">
                <el-form-item :rules="validation(null, 1)" prop="education_institute_id" :label="ruleForm.education_type == 4 ? $t('Global.azhar_area') : $t('Global.administration')">
                  <el-select :disabled="disabled" clearable filterable :no-data-text="$t('Global.nodata')" style="width: 100%" v-model="ruleForm.education_institute_id" :placeholder="ruleForm.education_type == 4 ? $t('Global.azhar_area') : $t('Global.administration')">
              ...
              <CCol sm="4">
                <el-form-item :rules="validation(null, 1)" prop="type_id" :label="ruleForm.education_type == 4 ? $t('Global.orgFollow') : $t('Global.SCgender')">
                  <el-select clearable :no-data-text="$t('Global.nodata')" style="width: 100%" v-model="ruleForm.type_id" :placeholder="ruleForm.education_type == 4 ? $t('Global.orgFollow') : $t('Global.SCgender')">
              ...
              <CCol sm="4">
                <el-form-item :rules="validation(null, 1)" prop="school_gender_id" :label="ruleForm.education_type == 4 ? $t('Global.org_type') : $t('Global.school_gender_id')">
                  <el-select clearable :no-data-text="$t('Global.nodata')" style="width: 100%" v-model="ruleForm.school_gender_id" :placeholder="ruleForm.education_type == 4 ? $t('Global.org_type') : $t('Global.school_gender_id')">
              ...
              <CCol sm="4">
                <el-form-item :rules="!isAuth($store.getters['organization/representative']) ? null : validation(null, 0)" prop="school_code" :label="ruleForm.education_type == 4 ? $t('Global.azhar_school_code') : $t('Global.school_code')">
                  <el-input clearable suffix-icon="el-icon-code" v-model="ruleForm.school_code"></el-input>
                </el-form-item>
              </CCol>
              <CCol sm="4">
                <el-form-item :rules="!isAuth($store.getters['organization/representative']) ? null : validation(null, 0)" prop="school_building_number" :label="ruleForm.education_type == 4 ? $t('Global.azhar_building_number') : $t('Global.school_building_number')">
              ...
              <CCol sm="4">
                <el-form-item class="leftToRight left_right" :rules="!isAuth($store.getters['organization/representative']) ? null : validation($t('Validation.email'), 'email')" prop="email" :label="ruleForm.education_type == 4 ? $t('Global.azhar_email') : $t('user.email')">
              ...
              <!-- <CCol sm="4" v-if="ruleForm.education_type == 4">
                <el-form-item class="leftToRight left_right" :rules="!isAuth($store.getters['organization/representative']) ? null : validation($t('Validation.email'), 'email')" prop="email" :label="$t('Global.azhar_email')">
                  <el-input name="azhar_email" id="azhar_email" clearable suffix-icon="el-icon-message" v-model="ruleForm.azhar_email" :placeholder="$t('Global.azhar_email')"></el-input>
                </el-form-item>
              </CCol> -->
              <CCol sm="4">
                <el-form-item prop="graduation_count" :label="$t('Global.graduation_count')">
              ...
              <CCol sm="4">
                <el-form-item :rules="!isAuth($store.getters['organization/representative']) ? null : validation(null, 0)" :label="ruleForm.education_type == 4 ? $t('Global.orgCreatedAt') : $t('Global.date_in')" prop="date_in">
                  <el-date-picker clearable style="width: 100%" value-format="yyyy-MM-d" v-model="ruleForm.date_in" type="date" :placeholder="ruleForm.education_type == 4 ? $t('Global.orgCreatedAt') : $t('Global.date_in')"></el-date-picker>
                </el-form-item>
              </CCol>
    ...
    <el-dialog :title="$t('Global.addInfo')" :visible.sync="organizationDialog" width="50%">
      <el-form :model="editForm" label-position="top" status-icon size="large" ref="editForm" :rules="editRules" label-width="120px" class="demo-ruleForm">
        <el-form-item v-if="type == 'administration'" class="m-0" :rules="validation(null, 1)" prop="administration" :label="ruleForm.education_type == 4 ? $t('Global.azhar_area') : $t('Global.administration')">
          <el-input clearable :placeholder="ruleForm.education_type == 4 ? $t('Global.azhar_area') : $t('Global.administration')" v-model="editForm.administration">
...
export default {
  ...
  computed: {
    ...
    getStage: function () {
      if (this.ruleForm.education_type == 4) {
        return this.lookups.stages.filter(t => t.education_type_id == 2);
      } else {
        return this.lookups.stages.filter(t => t.education_type_id == 1);
      }
    },
    getType: function () {
      if (this.ruleForm.education_type == 4) {
        return this.lookups.types.filter(t => t.education_type_id == 2);
      } else {
        if (this.lookups) {
          return this.lookups.types.filter(t => t.education_type_id == 1);
        }
      }
    },
  },
  ...
  watch: {
    ...
    'ruleForm.education_type':function(val,newVal){
       if (newVal) {
        this.ruleForm.education_institute_id = null;
        this.ruleForm.directorate_id = null;
      }
    },
      ...
      if (val) {
        this.$store
          .dispatch('directorate/institutes', {
            ...
            type: this.ruleForm.education_type,
          })
          .then(_ => {
            this.disabled = false;
          });
    ...
    save(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          // if (this.ruleForm.azhar_email && this.ruleForm.education_type == 4) {
          //   let az_email = { azhar_email: this.ruleForm.azhar_email };
          //   query = { ...query, ...az_email };
          // }
          this.$store
