<template>
  <div>
    <span v-if="!$store.state.evaluationNotes.complete && [showConstants('string', 'REPORT_APPROVAL_TEAM_COMMITTEE'), showConstants('string', 'REPORT_APPROVAL')].includes(showType)"></span>
    <CCard
      v-else-if="
        showAction &&
        ...
        [showConstants('string', 'LEADER_APPROVAL'), showConstants('string', 'DEFUALT_APPROVAL'), showConstants('string', 'REPORT_APPROVAL'), showConstants('string', 'REPORT_APPROVAL_TEAM_COMMITTEE')].includes(showType)
      "
    >
      ...
      <CCardBody :class="$store.getters['submission/name'] && !$store.getters['submission/name'].name ? 'disabledAll' : ''">
        <div :class="$store.state.submission.orgActive ? 'disabledAll' : ''" v-if="!(showType == showConstants('string', 'REPORT_APPROVAL_TEAM_COMMITTEE'))" class="d-lg-flex justify-content-lg-between">
          ...
          <div class="d-flex" v-loading="$store.state.archive.generalLoad">
            <div :class="isLoad ? 'disabledAll' : ''" class="d-flex w-100 flex-wrap flex_gap" id="hasChildf">
              <div id="action1" :class="isLoad ? 'disabledAll' : ''" class="right">
                <small v-if="showType == showConstants('string', 'LEADER_APPROVAL')" class="small text-muted align-content-center">
                  <vue-countdown class="small text-muted mb-0" v-if="time" :time="time" :interval="100" v-slot="{ days, hours, minutes, seconds }">
                    {{ $t('Global.remainingVisitTime') }}：{{ days }} {{ $t('Global.days') }}, {{ hours }} {{ $t('Global.hours') }}, {{ minutes }} {{ $t('Global.minutes') }}, {{ seconds }} {{ $t('Global.seconds') }}
                  </vue-countdown>
                </small>
                <el-button
                  ...
                  icon="el-icon-check"
                  @click="dialogOpen(false, 1, true)"
                  round
                  size="mini"
                  class="mx-1 right_icon"
                  type="success"
                >
                  <template v-if="showType === showConstants('string', 'DEFUALT_APPROVAL') && (evaluatorCheck(['ORGANIZATION_TYPE']) || evaluatorCheck(['LEADER_TYPE']))">
                    {{  evaluatorCheck(['LEADER_TYPE']) ? $t('Global.accept') : $t('Global.ensure') }}
                  </template>
                  <template v-else>
                    {{ showType == showConstants('string', 'REPORT_APPROVAL') && $store.state.submission.last_alert ?  evaluatorCheck(['LEADER_TYPE']) ? $t('Global.accept') : $t(`Global.approvals_status_${$store.state.submission.last_alert.actual_level}_ensure`) : $t('Global.accept') }}
                    <!-- <small v-else></small> -->
                  </template>
                </el-button>
                <el-button v-else icon="el-icon-check" @click="openDesicionDialog('add')" round size="mini" class="mx-1 right_icon" type="success">
                  <template v-if="$store.state.evaluationNotes.complete && $store.state.submission.last_alert && $store.state.submission.last_alert.allow_update == updateConstants('string', 'TAKE_DECISION')">
                ...
                <el-button
                  v-if="!evaluatorCheck(['LEADER_TYPE']) && $store.state.submission.last_alert.allow_update != updateConstants('string', 'TAKE_DECISION') && $store.state.submission.last_alert.evaluator_type != evConstants('ORGANIZATION_TYPE') && showType != showConstants('string', 'LEADER_APPROVAL')"
                  icon="el-icon-close"
                  ...
                  type="danger"
                >
                  {{ showType == showConstants('string', 'REPORT_APPROVAL') && $store.state.submission.last_alert ? $t(`Global.refuse_status_${$store.state.submission.last_alert.actual_level}`) : $t('Global.refuse') }}
                </el-button>
                <span
        ...
        <div v-else class="w-100">
          <Team label="team" @action="handleAccept" v-if="($store.state.evaluationNotes.complete && $store.getters['evaluation/team'] && $store.state.submission.last_alert.evaluator_type == evConstants('TEAM_TYPE')) || hasPermission('stage_update')" :team="$store.getters['evaluation/team']"></Team>
          <Team
            label="committee"
            @action="handleAccept"
            v-if="($store.state.evaluationNotes.complete && $store.getters['evaluation/committees'] && $store.state.submission.last_alert.evaluator_type == evConstants('COMMITTEE_TYPE')) || hasPermission('stage_update')"
            :team="$store.getters['evaluation/committees']"
          ></Team>
        </div>
      </CCardBody>
    </CCard>
    <el-dialog :title="$t('Global.writeNote')" :visible.sync="dialogVisible" width="80%">
      <div>
        <el-form label-position="top" :model="form" :rules="rules" status-icon size="large" ref="form" label-width="120px" class="stateForm demo-ruleForm">
          <CRow>
            <el-form-item :rules="accept ? { rule: false } : validation(false, 'textarea')" class="custom_area" style="width: 100%" prop="comment">
              <el-input id="comment" class="textarea" type="textarea" :placeholder="accept ? $t('Global.writeNote') : $t('Global.rejectReason')" v-model="form.comment"></el-input>
            </el-form-item>
          </CRow>
        </el-form>
        <span slot="footer" :class="isLoad ? 'disabledAll' : ''" class="dialog-footer" style="display: flex">
          <el-button icon="el-icon-close" @click="accept ? handleAccept() : sendReject('form')" round size="mini" class="mx-1 right_icon" :type="accept ? 'primary' : 'danger'">{{ $t('Global.accept') }}</el-button>
          <el-button
            class="ml-1 mr-1"
            @click="
              dialogVisible = false;
    ...
    <el-dialog append-to-body custom-class="body_dialog" width="70%" :title="$t(`Global.ensureAndDesicion`)" :visible.sync="dialogDecision">
      <el-form label-position="top" :model="ruleForm" status-icon size="large" ref="ruleForm" label-width="120px" class="demo-ruleForm">
        ...
        <el-form-item :rules="[{ required: true, message: `${$t('Validation.selectFiled')}`, trigger: 'change' }]" :label="$t('Global.decision_date')" prop="start">
          <el-date-picker @change="onFromPickValidation" format="yyyy-MM-d" value-format="yyyy-MM-d" clearable style="width: 100%" v-model="ruleForm.start" type="date" :placeholder="$t('Global.decision_date')"></el-date-picker>
        </el-form-item>
        <el-form-item :rules="[{ required: true, message: `${$t('Validation.selectFiled')}`, trigger: 'change' }]" :label="$t('Global.expiration_date')" prop="expiration">
          <el-date-picker @change="onPickValidation" format="yyyy-MM-d" value-format="yyyy-MM-d" clearable style="width: 100%" v-model="ruleForm.expiration" type="date" :placeholder="$t('Global.expiration_date')"></el-date-picker>
        </el-form-item>
  ...
  methods: {
    ...
    handleAccept(data) {
      this.$confirm(`${data ? this.$t('Global.teamConfirm') : this.$t('Global.ensureBeforeRequest')}`, {
        ...
        let query = { status: this.constants('PRE_APPROVE_STATUS'), note: this.form.comment ? String(this.form.comment) : '' };
        this.isLoad = true;
        let system;
        if (this.$store.state.submission.last_alert && (this.$store.state.submission.last_alert.show_type == this.showConstants('string', 'REPORT_APPROVAL') || this.$store.state.submission.last_alert.show_type == this.showConstants('string', 'REPORT_APPROVAL_TEAM_COMMITTEE'))) {
          system = 2;
