  ...
  <div class="custom_info_alert">
      ...
      <div class="fab-wheel" v-if="$store.getters['submission/team'] || $store.getters['submission/representative']">
        <template v-if="$store.getters['submission/team'] && $store.getters['submission/team'].all_members">
          <router-link
            ...
            :to="{ path: `/school/messages/${$route.params.submission_id}/${member.id}` }"
        ...
        <router-link v-if="$store.getters['submission/representative'] && !evaluatorCheck(['ORGANIZATION_TYPE'])" class="my-2 fab-action px-2 custom_link_white" :to="{ path: `/school/messages/${$route.params.submission_id}/${$store.getters['submission/representative'].id}` }">
    ...
    <el-alert v-if="!$store.getters['submission/decision'] && $store.state.submission.orgInfoRequired && evaluatorCheck(['ORGANIZATION_TYPE'])" effect="dark" class="mb-2 animate" :title="$t('Global.SystemManagement')" type="error" show-icon>
      <div class="d-flex">
        ...
        <router-link class="text-white custom_link" :to="{ path: `/school/organization/edit/${$store.state.submission.submission.organization_id}` }">
          <div style="margin: 0 5px">{{ $t('Global.pressNow') }}</div>
        </router-link>
      </div>
    </el-alert>
    <el-alert
      v-if="(!$store.getters['submission/decision'] || ($store.getters['submission/decision'] && $store.state.submission.decision.removed != 0)) && !$store.getters['submission/submissionLoad'] && lastAlert && !isLoad"
      effect="dark"
      class="mb-2 animate"
    ...
    >
      <div>
        ...
        <router-link v-if="showAction && showType == showConstants('string', 'SUBMISSION_UPDATE')" class="text-white custom_link" :to="{ path: `/school/submissions/edit/${$route.params.submission_id}/1` }">
          <div style="margin: 0 5px">{{ $t('Global.pressNow') }}</div>
        </router-link>
        <router-link v-if="showAction && showType == showConstants('string', 'FINANCE_UPDATE')" class="text-white custom_link" :to="{ path: `/school/submissions/edit${$route.params.id ? '/' + $route.params.id : ''}/${$route.params.submission_id}/2` }">
          <div style="margin: 0 5px">{{ $t('Global.pressNow') }}</div>
        </router-link>
        <router-link v-if="showAction && (showType == showConstants('string', 'TEAM_UPDATE') || showType == showConstants('string', 'VISIT_UPDATE'))" class="text-white custom_link" :to="{ path: `/school/submissions/edit/${$route.params.submission_id}/3` }">
          <div style="margin: 0 5px">{{ $t('Global.pressNow') }}</div>
        </router-link>
      </div>
    </el-alert>
    ...
    <Show id="submission_id" submission :loading="$store.getters['submission/submissionLoad']" icon="el-icon-message" :items="$store.getters['submission/submission']" :name="$t('Global.order')" except="team" v-if="hasPermission('access_submissions')">
      <template #show_action v-if="$store.state.user.allowPu || hasPermission('admin_system')">
        <!-- <el-button
          ...
          :disabled="(lastAlert.type == 1 && lastAlert.level != 6) || (evaluatorCheck(['TEAM_TYPE']) && lastAlert.type == 2) || evaluatorCheck(['COMMITTEE_TYPE'])"
          :type="$store.state.submission.is_disability ? 'success' : 'danger'"
          size="mini"
          class="mx-1 right_icon print_hide"
          @click="changeDisabilityStatus($route.params.id)"
          round
          >{{ $store.state.submission.is_disability ? $t('Global.disabilityBtn') : $t('Global.disabilityNoBtn') }}
        </el-button> -->
        ...
        <el-tooltip v-if="hasPermission('admin_system') || evaluatorCheck(['COMMITTEE_TYPE', 'PRESIDENT_TYPE'])" :content="$store.state.submission.is_disability ? $t('Global.disabilityBtn') : $t('Global.disabilityNoBtn')" placement="top">
          <el-switch @change="changeDisabilityStatus($route.params.id)" :value="$store.state.submission.is_disability ? true : false" active-color="#13ce66" inactive-color="#ddd" class="mx-1"> </el-switch>
        </el-tooltip>
        <router-link v-if="hasPermission('decision_approval')" class="mx-1" :to="`/school/submissions/council/${$route.params.id}`">
        ...
        <CDropdown 
         v-if="
          ...
          "
         color="success" placement="bottom-end" size="sm" class="d-inline customDrop print_hide">
          ...
          <router-link v-if="hasPermission('data_update') || (showAction && showType == showConstants('string', 'SUBMISSION_UPDATE'))" class="iF_found item d-block" :to="{ path: `/school/submissions/edit/${$route.params.submission_id}/1` }">
            <div style="text-align: initial"><i class="el-icon-edit"></i> {{ $t('Global.editSubmission') }}</div>
          </router-link>
          <router-link class="iF_found item d-block" v-if="evaluatorCheck(['ORGANIZATION_TYPE']) || hasPermission('data_update')" :to="{ path: `/school/submissions/additional/edit/${$route.params.submission_id}/${educationType}/2` }">
            <div style="text-align: initial"><i class="el-icon-edit"></i> {{ $t('Global.editAdditionalData') }}</div>
          </router-link>
          <router-link v-if="hasPermission('finance_update') || (showAction && showType == showConstants('string', 'FINANCE_UPDATE'))" class="iF_found item d-block" :to="{ path: `/school/submissions/edit/${$route.params.id}/${$route.params.submission_id}/2` }">
          ...
          <router-link v-if="hasPermission('team_update') || (showAction && (showType == showConstants('string', 'TEAM_UPDATE') || showType == showConstants('string', 'VISIT_UPDATE')))" class="iF_found item d-block" :to="{ path: `/school/submissions/edit/${$route.params.submission_id}/3` }">
            <div style="text-align: initial">
      ...
      <template #show_status>
        <router-link v-if="$store.getters['submission/name']" :to="{ path: $store.getters['submission/name'].id ? `/school/organization/${$store.getters['submission/name'].id}` : '' }">
        ...
        <CBadge v-if="!evaluatorCheck(['ORGANIZATION_TYPE']) && $store.state.submission.last_alert && $store.state.submission.last_alert.title" :color="$store.state.submission.last_alert.status == 0 ? 'danger' : 'info'" style="margin: 4px">
          {{ hasDecision ? $store.state.submission.decision.name : $store.state.submission.last_alert.title }}
        </CBadge>
        <CBadge v-if="evaluatorCheck(['ORGANIZATION_TYPE']) && $store.state.submission.last_alert && $store.state.submission.last_alert.title" :color="$store.state.submission.last_alert.status == 0 ? 'danger' : 'info'" style="margin: 4px">
      ...
      <template #other_data>
        ...
        <tr v-if="$store.getters['submission/submission']" :class="isLoad ? 'disabledAll' : ''">
          <th scope="row" width="20%">{{ educationType == 4 && currentUrl.includes('school') ? $t('Global.authorized_person_azhar') : educationType != 4 && currentUrl.includes('school') ? $t(`Global.school_manager`) : $t('Global.authorized_person') }}</th>
          <td width="80%">
      ...
      <template #bottom_other_data>
        <tr v-if="$store.getters['submission/submission']" :class="isLoad ? 'disabledAll' : ''">
          ...
          <td width="80%">
            <span>{{ $store.state.submission.submission ? $store.state.submission.submission.institute : '' }}</span>
          </td>
        </tr>
        <tr v-if="$store.getters['submission/submission']" :class="isLoad ? 'disabledAll' : ''">
          <th scope="row" width="20%">{{ $t('Global.directorate') }}</th>
          <td width="80%">
            <span>{{ $store.state.submission.submission ? $store.state.submission.submission.directorate : '' }}</span>
          </td>
        </tr>
          ...
          <td width="80%">
            <span>
              <div class="teams_state d-md-flex flex-wrap">
                <router-link v-if="lastAlert && !evaluatorCheck(['ORGANIZATION_TYPE', 'COMMITTEE_TYPE'])" class="custom-link" slot="reference" :to="{ path: `/school/team/${$store.getters['submission/team'].id}` }">
                  <CBadge class="custom-link ml-2 mr-2 text-wrap" color="info">{{ $store.getters['submission/team'].name }}</CBadge>
                  <CBadge class="custom-link ml-2 mr-2 text-wrap" color="info"> {{ $t('Global.reservation_members') }}</CBadge>
                </router-link>
                <div class="mb-2">
                ...
                <div class="d-inline teams_state" v-if="evaluatorCheck(['ORGANIZATION_TYPE']) && ($store.state.user.allowPu || hasPermission('admin_system'))">
                  <div v-if="showAction && showType == showConstants('string', 'MEMBER_APPROVAL')">
              ...
              >
                <div class="teams_state">
                  <div class="d-inline" v-if="showAction && showType == showConstants('string', 'MEMBER_APPROVAL') && ($store.state.user.allowPu || hasPermission('admin_system'))">
                    <el-tooltip v-if="member.excused === 2 && isAuth(member)" content="رفض الانضمام للفريق" placement="top-start">
                      <el-button @click="dialogOpen(false, member.id, $store.getters['submission/team'].id)" icon="el-icon-close" round size="mini" class="mx-1 right_icon" type="danger">{{ $t('Global.refuse') }}</el-button>
                    </el-tooltip>
                    <el-tooltip v-if="member.excused === 2 && isAuth(member)" content="الموافقة على الانضمام للفريق" placement="top-start">
                      <el-button icon="el-icon-check" @click="handleMemberAccept(member.id, $store.getters['submission/team'].id)" round size="mini" class="mx-1 right_icon" type="success">{{ $t('Global.accept') }}</el-button>
                    </el-tooltip>
                  </div>

                  <template
                    v-if="
                      (!evaluatorCheck(['ORGANIZATION_TYPE']) && (evaluatorCheck(['PRESIDENT_TYPE', 'ACCREDITATION_TYPE', 'VICE_TYPE', 'MANAGER_TYPE']) || hasPermission('admin_submissions'))) ||
                  ...
                  >
                    ...
                    <template v-if="$store.state.user.allowHr || hasPermission('admin_system')">
                      <router-link
                        class="print_hide"
                        v-if="member.id != authUser().id && (evaluatorCheck(['TEAM_TYPE']) || (evaluatorCheck(['ORGANIZATION_TYPE']) && member.id != authUser().id && member.team_job != 5))"
          ...
          <el-table-column prop="" label=" ">
            <template slot-scope="scope">
              <div :style="$i18n.locale === 'ar' ? 'text-align:left' : 'float: right;'">
                <router-link :to="`/school/submissions/submission/${scope.row.id}/${scope.row.evaluation_id}`" class="table-action-item">
