        ...
        <CCardBody :class="isLoad ? 'disabledAll' : ''" dir="rtl" style="text-align: initial">
          <el-form @submit.native.prevent label-position="top" :model="ruleForm" :rules="rules" status-icon size="large" ref="ruleForm" label-width="120px" class="demo-ruleForm">
            <CRow>
              ...
              <!-- <CCol :sm="hasPermission('admin_organizations') ? '6' : '4'" class="direction d-flex flex-wrap flex-column">
                <el-form-item :label="$t('Global.visitTypes')" :rules="validation(null, 1)" prop="visit_type_id">
              ...
              <el-divider class="mb-3 mt-3" content-position="center"><i class="el-icon-map-location"></i> {{ $route.params.type == 4 ? $t('Global.azharManagerData') : $t('Global.managerData') }}</el-divider>
              <CCol sm="6">
              ...
              <CCol sm="6">
                <el-form-item :rules="validation(null, 0)" :label="$t('Global.authorized_address')" prop="authorized_address">
                  <el-input :placeholder="$t('Global.authorized_address')" v-model="ruleForm.authorized_address"></el-input>
                </el-form-item>
              </CCol>
              <CCol sm="6">
                <el-form-item :label="$t('Global.national_id')" prop="authorized_national_id">
                  <el-input :maxlength="14" :placeholder="$t('Global.national_id')" v-model="ruleForm.authorized_national_id"></el-input>
                </el-form-item>
              </CCol>
              <CCol sm="6">
                <el-form-item :rules="validation(null, 0)" :label="$t('Global.authorized_education')" prop="authorized_education">
                  <el-input :placeholder="$t('Global.authorized_education')" v-model="ruleForm.authorized_education"></el-input>
                </el-form-item>
              </CCol>
              <CCol sm="6">
                <el-form-item :rules="validation(null, 0)" :label="$t('Global.authorized_job')" prop="authorized_job">
                  <el-input :placeholder="$t('Global.authorized_job')" v-model="ruleForm.authorized_job"></el-input>
                </el-form-item>
              </CCol>
              <CCol sm="6">
...
export default {
  ...
  watch: {
    'ruleForm.authorized_national_id'(value) {
      if (value) {
        this.ruleForm.authorized_national_id = value.toString().replace(/[^0-9]/g, '');
      }
    },
    ...
    init() {
      this.$store
        .dispatch('submission/SCedit', {
          ...
          this.ruleForm.authorized_address = this.$store.state.submission.authorized_address ? this.$store.state.submission.authorized_address : null;
          this.ruleForm.authorized_national_id = this.$store.state.submission.authorized_national_id ? this.$store.state.submission.authorized_national_id : null;
          ...
          this.ruleForm.authorized_education = this.$store.state.submission.authorized_education ? this.$store.state.submission.authorized_education : null;
          this.ruleForm.authorized_job = this.$store.state.submission.authorized_job ? this.$store.state.submission.authorized_job : null;
