        ...
        <CCard style="width: 100%">
            <CCardBody class="text-initial">
                ...
                <el-form label-position="top" :model="ruleForm" status-icon size="large" ref="ruleForm"
                         label-width="120px">
                    <CRow>
                        <CCol sm="12" lg="8" class="mt-3">
                            <el-form-item :rules="validation(false, 'textarea')" prop="message"
                                          class="h-40 custom_text_area">
                                <el-input autosize resize="none" type="textarea"
                                          :placeholder="$t('Global.ticketMessage')"
                                          v-model="ruleForm.message"></el-input>
                            </el-form-item>
                        </CCol>
                        <CCol sm="12" lg="2" class="mt-3" v-if="!hasPermission('access_tickets')">
                            <el-form-item class="mb-3" prop="type"
                                          :rules="!hasPermission('access_tickets')? validation(null, 1) : {}">
                                <el-select :no-data-text="$t('Global.nodata')" v-model="ruleForm.type"
                                           :placeholder="$t('Global.sendTo')" style="width: 100%">
                        ...
                        <CCol lg="1" sm="12" class="d-flex align-items-center align-self-start mt-lg-3">
                            <el-form-item :show-message="photoError"
                                          :error="photoError == 1 ? 'برجاء رفع صورة البطاقة الشخصية ' : 'برجاء التأكد من نوع الملف'"
                                          prop="national_id_photo" class="mb-0 left_right">
                                <div :class="`${errorFile ? 'shake' : ''}`" class="upload-btn-wrapper">
                                    ...
                                    <el-tooltip content="jpg , png , jpeg , pdf" placement="top-start">
                                        <el-button style="overflow: hidden; max-width: 100px; position: relative" round
                                                   size="small" type="primary" class="no_hover_effect right_icon">
                                            <input type="file" multiple style="height: 33px; width: 100%"
                                                   accept=".jpg,.png,.jpeg,.pdf" id="fileInput" @change="upload"
                                                   ref="file"/>
                        ...
                        <CCol lg="1" sm="12" class="d-flex align-items-center align-self-start mt-lg-3">
                            <button class="mx-1 el-button el-button--default is-round sec_color"
                                    @click="addTicket('ruleForm')" type="reset" style="padding: 0.6rem 1rem">
            ...
            <CCardBody v-loading="$store.state.ticket.load">
                <div class="custom_report mb-3 pb-2">
                    <div v-for="ticket in $store.getters['ticket/tickets']">
                                ...
                                <div class="review_header my-0" style="margin-left: 10px; margin-right: 10px">
                                    ...
                                    <div>
                                        ...
                                        <h6 class="custom_badge">
                                            <i class="el-icon-time"></i><span style="font-size: 11px"
                                                                              class="localTime mr-1 ml-1">{{ toLocalDatetime(ticket.created_at, `${$i18n.locale == 'ar' ? 'ar' : 'en'}`) }}</span>
                                            <CBadge color="info badge-pill" class="m-1" style="direction: ltr">{{
                                                ticket.diff }}
                                            </CBadge>
                                        </h6>
                                    </div>
                        ...
                        <div :class="ticket.status == 1 ? 'mx-5' : ''" class="report_style button_after"
                             style="margin-left: 20px; margin-right: 20px">
                            <p class="mb-0 text-initial" style="word-break: break-word">{{ ticket.message }}</p>
                            <div class="d-flex flex-wrap">
                                <div v-for="document in ticket.documents"
                                     class="pointer d-flex custom_link mx-1 text-initial">
                                    <el-image style="width: 100px; height: 100px" v-if="isImage(document.extension)"
                                              :src="document.url"
                                              :preview-src-list="srcList(ticket.documents)"></el-image>
                ...
                <div class="d-flex justify-content-center">
                    <el-button round size="mini" @click="More" class="right_icon custom_send_icon" type="primary">{{
                        $t('Global.more') }}
                    </el-button>
                </div>
            </CCardBody>
...
<script>
    export default {
        ...
        methods: {
            ...
            Refresh() {
                this.$store.dispatch('ticket/get', {query: {limit: this.limit}, id: this.id});
            },
            More() {
            ...
            addTicket(formName) {
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        ...
                        this.isLoad = true;
                        this.$store
                            .dispatch('ticket/add', {query: formData, id: this.id})
                            .then(() => {
                                this.$notify({
                                    type: 'success',
