<template>
  <div v-if="hasPermission('access_teams')" id="print">
    ...
    <Show :loading="$store.getters['team/teamLoad']" icon="el-icon-user" :items="$store.getters['team/team']" @init="init" :name="$store.getters['team/name']" v-has-permissions="'show_teams'">
      <template #show_action>
        <template v-if="$store.state.user.allowHr || hasPermission('admin_system')">
          ...
          <i class="el-icon-printer mr-2 pointer print_hide" style="font-size: 17px" @click="print('print', $store.getters['team/name'])"></i>
          <router-link class="print_hide mr-2" v-if="hasPermission('admin_teams')" :to="{ path: `/higher/team/edit/${$route.params.id}`.toString() }">
            <CIcon name="cil-pencil" color="bg-primary"></CIcon>
          </router-link>
          <el-button :class="load ? 'disabledAll' : ''" v-if="hasPermission('edit_hotels')" @click="openReservationDialog($store.state.team.reservation)" round size="mini" class="mx-1 p-2 right_icon" style="font-size: 13px" type="primary">
            <i class="el-icon-circle-plus"></i>
            {{ $store.state.team.reservation ? $t('Global.edit_reservation') : $t('Global.add_reservation') }}
          </el-button>
          <el-button :class="load ? 'disabledAll' : ''" v-if="hasPermission('reservation_approval') && $store.state.team.status" @click="approve($store.state.team.submission)" round size="mini" class="mx-1 p-2 right_icon" style="font-size: 13px" type="success">
      ...
      <template #bottom_other_data>
        <tr v-if="$store.state.team.team">
          ...
          <td width="80%">
            <span v-if="member" v-for="member in $store.state.team.team.all_members" style="margin: 5px">
              <router-link :to="{ path: `/users/${member.user_id}` }" v-if="$store.getters['evaluation/evaluator'] !== evConstants('TEAM_TYPE')">
              ...
              <router-link :to="{ path: `/users/${member.user_id}` }" v-else-if="$store.getters['evaluation/evaluator'] == evConstants('TEAM_TYPE') && member.team_job != constants('COMMITTEE')">
                {{ member.ar_name }} ({{ member.job }})
                <CBadge color="warning" v-if="member.excused === 1">{{ $t('Global.excused') }}</CBadge>
                <CBadge color="info" v-if="member.excused === 0">{{ $t('Global.exist') }}</CBadge>
              </router-link>
        ...
        <tr v-if="$store.getters['team/reservation'] && $store.state.team.reservation">
          ...
          <td width="80%" v-if="$store.state.team.reservation.hotel">
            <div class="mb-2" style="cursor: pointer" @click="$router.push(`/higher/hotel/${$store.getters['team/reservation'].hotel.id}`)">
    ...
    <Show v-if="$store.getters['team/submission']" :loading="$store.getters['team/teamLoad']" icon="el-icon-user" :items="$store.getters['team/submission']" :name="$t('Global.order')">
      <template #show_status>
        <router-link v-if="$store.state.team.submission" :to="hasPermission('access_organizations') ? { path: $store.state.team.submission.organization_id ? `/higher/organization/${$store.state.team.submission.organization_id}` : '' } : ''">
          {{ $store.state.team.submission.name ? $store.state.team.submission.name : $store.state.team.submission.temp_organization_name ? $store.state.team.submission.temp_organization_name : '' }}
        </router-link>
      </template>
      <template #other_data>
      ...
      <template #bottom_other_data>
        <tr>
          ...
          <td width="80%" v-if="$store.getters['team/representative']">
            <router-link class="custom-link" :to="{ path: `/users/${$store.getters['team/representative'].id}` }">
    ...
    <CCard v-loading="$store.getters['team/teamLoad']" v-if="$store.getters['team/reservation']">
      <CCardHeader>
        ...
        <div :class="$i18n.locale === 'en' ? 'card-header-actions' : ''" style="display: flex; justify-content: flex-end; align-items: center">
          <template v-if="$store.state.user.allowHr || hasPermission('admin_system')">
            <el-button v-if="hasPermission('edit_teams')" @click="openServiceDialog" round size="mini" class="mx-1 p-2 right_icon" style="font-size: 13px" type="primary">
              <i class="el-icon-circle-plus"></i>
              {{ $t('Global.addService') }}
            </el-button>
          </template>
      ...
      <CCardBody>
        <el-table v-if="$store.getters['team/reservation'] && $store.getters['team/reservation'].services" :data="servicesData" style="width: 100%" :header-cell-class-name="$i18n.locale === 'ar' ? 'text-right' : ''" :cell-class-name="$i18n.locale === 'ar' ? 'text-right' : ''">
    ...
    <el-dialog :title="$t('route.services')" :visible.sync="service_dialog" class="p-0" width="60%" v-if="$store.state.team.team">
            ...
            <CCol sm="12">
              <el-form-item :rules="validation(null, 1)" prop="employee" :label="$t('Global.member')">
                <el-select filterable style="width: 100%" v-model="ruleForm.employee" :placeholder="$t('Global.member')">
                  <el-option v-if="$store.state.team.team" v-for="member in members" :key="member.user_id" :label="member.ar_name" :value="member.user_id">
            ...
            <CCol sm="4">
              <el-form-item :rules="validation(null, 1)" :label="$t('Global.roomType')" prop="service_type">
                <el-select filterable :no-data-text="$t('Global.nodata')" style="width: 100%" v-model="ruleForm.service_type" :placeholder="$t('Global.roomType')">
                  <el-option v-for="item in $store.getters['team/services']" :key="item.id" :label="item.name" :value="item.id">
                    <span style="float: right; color: #8492a6; font-size: 13px">
                      {{ item.name }}
                    </span>
                  </el-option>
    ...
    <el-dialog :title="$t('Global.add_reservation')" :visible.sync="reservation_dialog" class="p-0" width="60%">
      <CCardBody dir="rtl" style="text-align: initial" class="p-0">
        <el-form label-position="top" :model="reservationForm" status-icon size="large" ref="reservationForm" label-width="120px" class="demo-ruleForm">
          <CRow>
            <CCol sm="12">
              <el-form-item :rules="validation(null, 1)" prop="hotel_id" :label="$t('Global.hotel')">
                <el-select filterable style="width: 100%" v-model="reservationForm.hotel_id" :placeholder="$t('Global.hotel')">
                  <el-option v-for="hotel in $store.state.team.hotels" :key="hotel.id" :label="hotel.name" :value="hotel.id">
                    <span style="float: right; color: #8492a6; font-size: 13px">
                      {{ hotel.name }}
                    </span>
                  </el-option>
            ...
            <CCol sm="12">
              <el-form-item :label="$t('Global.topic')" style="width: 100%" prop="topic">
                <el-input autosize type="textarea" :placeholder="$t('Global.topic')" v-model="reservationForm.topic"></el-input>
              </el-form-item>
            </CCol>
          </CRow>
        </el-form>
      ...
      <template v-if="$store.state.user.allowHr || hasPermission('admin_system')">
        <span slot="footer" class="dialog-footer" :class="load ? 'disabledAll' : ''">
          <el-button @click="saveReservation('reservationForm')" @keyup.enter="saveReservation('reservationForm')" type="primary">{{ $t('Global.add') }}</el-button>
  ...
  methods: {
          ...
          if (this.$store.state.team.reservation) {
            ...
            this.$store
              .dispatch('hotel/saveService', {
                ...
                query: { ...this.ruleForm, hotel_reservation_id: this.$store.state.team.reservation.id },
              })
              .then(_ => {
    ...
    approve(submission) {
      if (submission) {
        this.load = true;
        this.$store
          .dispatch('hotel/approve', submission.id)
          .then(_ => {
            this.init();
            this.load = false;
            this.$notify({
