  ...
  <div class="custom_info_alert custom_alert">
    ...
    <el-card class="box-card" :class="isLoad ? 'disabledAll' : ''">
      <div slot="header" class="d-flex justify-content-between align-items-center">
        <span class="d-block w-3 text-initial">{{ $t('Global.FilesNeeded') }}</span>
        <CSpinner v-if="isLoad" size="sm" color="info" />
        <div v-if="$store.state.submission.docsInfo && $store.getters['submission/canUpload'] && ($store.state.submission.docsInfo.need > 0)">
        ...
        <div v-for="(document, index) in parent.children" class="p-1 border mb-2 text item docs d-flex justify-content-between align-items-center">
          ...
          <div class="d-flex">
            <span class="ml-2 mr-2" :class="!checkIfExist($store.getters['submission/MainUploadsDocuments'], document.short_name) ? 'text-danger' : ''">
              ...
              {{ getDocSize($store.getters['submission/MainUploadsDocuments'], document.short_name) }}
            </span>
      ...
      <div v-for="(parent, index) in $store.getters['submission/allDocuments']">
        <div class="docs_header d-flex justify-content-between align-items-center">
          <span :style="parent.mandatory !== 1 ? 'direction: ltr;' : ''"
            ...
            <small class="small text-muted align-content-center" v-if="parent.mandatory !== 1 && $store.getters['submission/extraFile'] && $store.getters['submission/extraFile'].status">
            ...
            <small v-else></small>
            <!-- v-if="
                parent.short_name == 'ORGANIZATION_DOCUMENT' &&
            ...
            <el-collapse @change="changeArrow(index + 1)" class="custom_collapse custom_collapse_docs" v-model="activeName" accordion>
              <el-collapse-item :title="document.title">
                ...
                <div class="p-1 mt-2 text docs d-flex justify-content-between align-items-center" v-for="(subDocument, index) in document.children" v-if="subDocument.type === 0">
                  <div class="d-flex">
                    <a download class="pointer custom_link custom_tooltip" :class="subDocument.mandatory == 1 ? 'el-form-item is-required el-form-item--small p-0' : ''" target="_blank" :href="getDocUrl($store.getters['submission/documents'], subDocument.short_name)"
...
export default {
  ...
  computed: {
    ...
    //   ifDone(){
    //       if(this.$store.getters['submission/allDocuments'].length){
    //           if(this.$el.querySelector('#parent_false')) return false;
    //            return true
    //       }
    //   }
  },
