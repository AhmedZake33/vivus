      ...
      <div v-for="member in team" class="round p-1 border mr-1 ml-1 mb-2 text custom_item docs d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex align-items-center ml-1 mr-1">
          ...
          <div v-else-if="!member.approved && member.can" class="d-flex">
            <el-button :disabled="!$store.state.evaluationNotes.complete" v-if="label == 'committee'" @click="handleAcceptWithMessage"  round size="mini" class="mr-2 ml-2" type="success">{{ $t(`Global.accept`) }} </el-button>
            <el-button :disabled="!$store.state.evaluationNotes.complete" v-else @click="handleAccept" round size="mini" class="mr-2 ml-2" type="success">{{ $t(`Global.accept`) }} </el-button>
          </div>
