        ...
        <el-form label-position="top" :model="editFormData" status-icon size="large" :rules="editFormDataRules" ref="editFormData" label-width="120px" class="demo-ruleForm team_form">
          <CRow>
            <CCol sm="6">
              ...
              <!-- <el-form-item prop="visitGroups" :label="$t('Global.visit')">
                              <el-select :disabled="$store.state.submission.check_status != 1" clearable :no-data-text="$t('Global.nodata')" style="width:100%" v-model="editFormData.visit" filterable @change="reset" ref="all" :placeholder="$t('Global.visit')">
                                <el-option v-for="item in $store.getters['submission/visitGroups']" :key="item.id" :label="item.name" :value="item.id">
                                  <span style="float: right; color: #8492a6; font-size: 13px">
                                    {{ item.name }}
                                  </span>
                                </el-option>
            ...
            <CCol sm="6">
              <!-- <el-form-item prop="teamName" :label="$t('Global.teamName')">
                              <el-input clearable :placeholder="$t('Global.teamName')" v-model="editFormData.teamName">
              ...
              <div style="position: relative" class="no_arrow">
                <el-form-item :rules="showType == showConstants('string', 'TEAM_UPDATE') || lastAlert.actual_level > 4 ? validation() : { required: false }" :label="$t('Global.presedent')" prop="leader">
                  <el-select popper-class="hide_chooses" :disabled="showType != showConstants('string', 'TEAM_UPDATE') && lastAlert.actual_level <= 4" :no-data-text="$t('Global.nodata')" style="width: 100%" v-model="editFormData.leader" :placeholder="$t('Global.presedent')">
              ...
              <div style="position: relative">
                <el-form-item :label="$t('Global.announce')" prop="observe">
                  <el-select filterable multiple popper-class="hide_chooses" :disabled="showType != showConstants('string', 'TEAM_UPDATE') && lastAlert.actual_level <= 4" :no-data-text="$t('Global.nodata')" style="width: 100%" v-model="editFormData.observe" :placeholder="$t('Global.announce')">
                    <div v-if="$store.getters['submission/selectedTeam']">
                      <el-option v-for="user in $store.getters['submission/selectedTeam'].all_members" :key="user.id" :label="user.ar_name" :value="user.id">
                        <span style="float: right; color: #8492a6; font-size: 13px">
                          {{ user.ar_name }}
                ...
                <div class="custom_left" v-if="showType == showConstants('string', 'TEAM_UPDATE') || lastAlert.actual_level > 4" @click="openDialog(3)"><i class="el-icon-circle-plus" style="font-size: 22px"></i></div>
              </div>
              <div class="members_hide_close no_arrow" style="position: relative">
...
export default {
  ...
  data() {
    return {
      ...
      editFormDataRules: {
        // teamName: [{ required: true, message: 'من فضلك يجب كتابة اسم الفريق   ', trigger: 'blur' }],
        ...
        // visit: [{ required: true, message: 'من فضلك يجب اختيار الزيارة  ', trigger: 'change' }],
      },
    };
  },
  watch: {
  ...
  methods: {
    ...
    activeSubmission(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.isLoad = true;
          this.$store
            .dispatch('submission/activeSubmission', {
              id: this.$route.params.id,
              query: query,
            })
            .then(() => {
    ...
    closeDialog(data) {
      // console.log("in close Team",data);
      // this.$store.commit('SET_LEADER', [{id:this.editFormData.leader}]);
      // this.editFormData.leader = this.$store.state.leader[0].id;
    ...
    saveTeam(data) {
      if (data.type == 1) {
        ...
        // this.editFormData.observe = data.users[0].id;
        // if (data.users[0].id == this.editFormData.observe) {
        //   this.selectedObserve[0].ar_name = data.users[0].ar_name;
        //   this.selectedObserve[0].id = data.users[0].id;
        // }
      } else if (data.type == 4) {
