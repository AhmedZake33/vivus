          ...
          <el-form label-position="top" :model="ruleForm" status-icon size="large" ref="ruleForm" label-width="120px" class="demo-ruleForm">
            <CRow>
              <div class="w-100">
                ...
                <CCol>
                  <el-form-item :rules="validation(false, 'textarea')" class="custom_area" :label="$t('Global.topic')" style="width: 100%" prop="topic">
                    <el-input autosize type="textarea" :placeholder="$t('Global.topic')" v-model="ruleForm.topic"></el-input>
                  </el-form-item>
                </CCol>
              </div>
            </CRow>
    ...
    init() {
      ...
      this.$store.dispatch('hotel/edit', payload).then(() => {
        if (this.$store.state.hotel.selected_reservation && this.$store.state.hotel.selected_reservation.team) {
          ...
          this.ruleForm.topic = this.$store.state.hotel.selected_reservation.topic;
        }
      });
    },
    add(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.isLoad = true;
          let query = { team_id: this.ruleForm.team_id, hotel_id: this.$route.params.id, topic: this.ruleForm.topic };
          this.$store
            .dispatch('hotel/addReservation', {
              query: query,
              id: this.$route.params.reservation_id ? this.$route.params.reservation_id : null,
            })
            .then(() => {
              this.$router.go(-1);
              this.isLoad = false;
